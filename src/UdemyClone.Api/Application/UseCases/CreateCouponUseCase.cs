...
namespace UdemyClone.Api.Application.UseCases;
...
public class CreateCouponUseCase
{
    ...
    public CreateCouponUseCase(
        IGenericRepository<Kupon> kuponRepo,
    ...
    public async Task<(bool Success, string? Error, Kupon? Coupon)> ExecuteAsync(int instructorId, CreateCouponRequest req)
    {
        ...
        var count = await _kuponRepo.Query().CountAsync(k => k.InstructorId == instructorId && k.CreatedAt >= since);
        ...
        if (req.CourseIds != null && req.CourseIds.Length > 0)
        {
            ...
            if (courses.Any(c => c.EgitmenId != instructorId))
